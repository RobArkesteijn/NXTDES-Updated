...
export default defineComponent({
  ...
  //       });
  //   },
  //   fetchUserListAndToggleFavorite() {
  //     const heart = document.getElementById('heart') as HTMLElement;
  ...
  //           this.indexList = Object.keys(data);
  //           console.log(this.indexList);
  //           const indexOfElement = this.list.indexOf(path);
  //           const indexNumber = this.indexList[indexOfElement];
  //           const newIdNumber = Number(this.indexList.at(-1)) + 1;
  //           if (!this.list.includes(path)) {
  //             heart.classList.add('active');
  //             this.addToList(newIdNumber);
  //             this.$notify({ type: 'success', text: 'Added to list'});
  //           } else {
  //             heart.classList.remove('active');
  //             this.deleteFromList(indexNumber);
  //             this.$notify({ type: 'warn', text: 'Removed from list'});
  //           }
  //         })
  //         .catch((error) => {
  //           console.log(error);
  //         })
  //     } else {
  //       this.$notify({ type: 'info', text: 'Please log in to add this Destination to your list'});
  //     }
  //   },
  //   addToList(index: number) {
  //     const user = getAuth().currentUser?.email?.replace(/[.#$\[\]]/g, "_");
  //     const userRef = dataRef(db, `users/${user}/sights/${index}`);
  //     set(userRef, `${this.$route.path.slice(1)}`)
  //       .then(() => {
  //         console.log('Data successfully added');
  //       })
  //       .catch((error) => {
  ...
  //       });
  //   },
  //   isFavoriteCheck(path: string, currentUser: User) {
  //     const heart = document.getElementById('heart') as HTMLElement;
  //     const user = currentUser.email?.replace(/[.#$\[\]]/g, "_");
  //     const userRef = dataRef(db, `users/${user}/sights`);
  //     get(userRef)
  ...
  //     onAuthStateChanged(getAuth(), (user) => {
	// 		if (user) {
	// 			this.isFavoriteCheck(path, user)
	// 		} else {
	// 			console.log('User is not logged in')
	// 		}
	// 	});
